...
class DriverWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.cursor.execute(
            f'select ((YEAR(now()) - YEAR(birth_date)) - ((DATE_FORMAT(now(), "00-%m-%d") < DATE_FORMAT(birth_date, '
            f'"00-%m-%d")))) as age from driver where driver_id = {self.driver_id}')
    ...
    def to_excel(self):
        ...
        for col_idx, header in enumerate(
                ["id маршрута", "авто", "водитель", "дата выезда", "дата прибытия", "расстояние",
                 "точка назначения"], start=1):
